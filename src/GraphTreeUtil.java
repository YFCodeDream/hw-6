...
public class GraphTreeUtil<T extends Comparable<T>> {
    /**
    ...
    /**
     * 绘制二叉树对外API
     * @param binarySearchTree 二叉树
     * @param fileName 保存文件名
     * @throws IOException 文件路径无效，则抛出异常
     */
    public void PaintTree(BinarySearchTree<T> binarySearchTree, String fileName) throws IOException {
        if (binarySearchTree == null) {
            System.out.println("The root of the tree is null");
            return;
        }

        totalLevel = binarySearchTree.height() - 1;
        ...
        ArrayList<BinarySearchTree<T>.Node> nodeList = new ArrayList<>();
        nodeList.add(binarySearchTree.getRoot());
     ...
     */
    private void GraphTreeByBFS(Graphics2D graphics2D,
                                ArrayList<BinarySearchTree<T>.Node> list,
                                int level,
                                boolean repaint) {
        ArrayList<BinarySearchTree<T>.Node> buffer = new ArrayList<>();
        for (int i = 0 ; i < list.size() ; i++) {
            BinarySearchTree<T>.Node tempNode = list.get(i);
            ...
            if (tempNode != null) {
                ...
                graphics2D.drawString(tempNode.value.toString() + " ", graphicNodeX, graphicNodeY);
     ...
     */
    private boolean isBufferEmpty(List<BinarySearchTree<T>.Node> list) {
        boolean result = true;
        for (BinarySearchTree<T>.Node node : list) {
            if (node != null) {
                result = false;
                break;
            }
